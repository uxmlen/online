...
try {
    ...
    case 1:
        ...
        std::cout << "New data received from " << msg_content_ << std::endl;
        break;
...
try {
    ...
    bind_status_ = bind(socket_file_descriptor,
        (struct sockaddr*)&serveraddress, sizeof(serveraddress));
    if (conn_ == -1) {
        std::cerr << YELLOW << "ERORR: Bind failed!" << RESET << std::endl;
        exit(1);
    }

    conn_status_ = listen(socket_file_descriptor, 5);
    if (conn_status_ == -1) {
        std::cerr << YELLOW << "error: server cannot get new data" << RESET
                  << std::endl;
        exit(1);
    }
    std::cout << GREEN << "server is waiting new connections" << RESET
              << std::endl;
    socket_len_ = sizeof(client);

    conn_ = accept(
        socket_file_descriptor, (struct sockaddr*)&client, &socket_len_);
    if (conn_ == -1) {
        std::cerr << "Server is unable to accept the data from client.!"
                  << std::endl;
        exit(1);
    }
